...
class Tester():
    ...
    def test_algo(self,algo,arms):
        ...
        conf = [conf for _ in range(tot_pulls)]
        ...
        return [conf, sim_num, pull_no, chosen_arms, rewards, cumulative_rewards]
...
if __name__ == '__main__':
    ...
    potential_confidence_values = [0.5,1,1.41,1.8,2]
    """
    Running Simulations 
    """
    fin = np.zeros((1,6))
    for conf in potential_confidence_values:

        alg = UCB(confidence=conf,n_arms=n_arms)
    ...
    print(alg.values)
    fin = fin[1:]
    df = pd.DataFrame(fin, columns=["Confidence", "Simulation", "Pull_no", "Chosen_arm", "Reward", "Cumul_Reward"])
